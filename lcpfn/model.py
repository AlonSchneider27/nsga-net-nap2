...
class LCPFN(torch.nn.Module):
    def __init__(self, model_name="EMSIZE512_NLAYERS12_NBUCKETS1000"):
        ...
        # VENDORED PATCH: the checkpoint is a pickled whole nn.Module saved on
        # torch<=1.11; torch>=2.6 defaults to weights_only=True and refuses it,
        # and CPU-only hosts need map_location. Upstream passes no kwargs.
        self.model = torch.load(
            getattr(lcpfn, model_name) if model_name in lcpfn.model_dict else model_name,
            map_location="cpu",
            weights_only=False,
        )
        # VENDORED PATCH: modules pickled on torch<=1.11 miss attributes that
        # torch>=2.0 forward() reads (nn.GELU grew `approximate` in 1.12).
